...
object DataBindingComponent {
    ...
    @BindingAdapter("textPah")
    @JvmStatic
    fun setText(textView: TextView, text: Any?) {
        if (text != null) {
            textView.text = format(text)
        } else {
            textView.text = ""
        }
    }

    @BindingAdapter("textHUT")
    @JvmStatic
    fun setHUTText(textView: TextView, text: Any?) {
        if (text != null) {
            textView.text = "开机率 : " + format(text) + "%"
        } else {
            textView.text = "开机率 : 0.0"
        }
    }


    @BindingAdapter("textUZN")
    @JvmStatic
    fun setUZNText(textView: TextView, text: Any?) {
        if (text != null) {
            textView.text = "稼动率 : " + format(text) + "%"
        } else {
            textView.text = "稼动率 : 0.0"
        }
    }


    @BindingAdapter("xtext", "xRes", requireAll = false)
    @JvmStatic
    fun setText(textView: TextView, text: String?, @StringRes res: Int) {
        if (!TextUtils.isEmpty(text)) {
            textView.text = text
        } else if (res != 0) {
            textView.text = getResources().getString(res)
        } else {
            textView.text = ""
        }
    }
    ...
    fun setBackgroundRes(v: View, drawableId: Int) {
        if (drawableId > View.NO_ID) v.setBackgroundResource(drawableId) else v.background = null
    }
    ...
    fun setImageDrawable(v: ImageView, @DrawableRes drawableId: Int) {
        if (drawableId > View.NO_ID) v.setImageResource(drawableId) else v.setImageDrawable(null)
    }
    ...
    fun setElevation(v: CardView, dp: Int) {
        v.cardElevation = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            dp.toFloat(),
            v.resources.displayMetrics
        )
