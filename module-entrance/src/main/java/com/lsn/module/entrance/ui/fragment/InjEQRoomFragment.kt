...
import com.pmisy.roomkb.databinding.FragmentInjEqRoomBinding
...
import com.pmisy.roomkb.ui.viewmodel.InjEQRoomViewModel
 ...
 */
@AndroidEntryPoint
class InjEQRoomFragment :
    BaseCoreFragment<InjEQRoomViewModel, FragmentInjEqRoomBinding>(R.layout.fragment_inj_eq_room) {
        ...
        viewModel.success.observe(this) {
            when (it.api) {
                ...
                ApiConstants.Comm.SEARCH_PROD_EQUIPMENT_DETAIL_LIST -> {
                    val kbEqRoomEntity = it.data as KBEqRoomEntity
                    val equipmentDetails = kbEqRoomEntity.list
                    equipmentDetails?.let {
                        val page = ArrayList<String>()
                        val splitList = splitList(equipmentDetails!!, getKanbanMaxCount())
    ...
    private fun getKanbanMaxCount(): Int {
        if (getKBRoomActivity() == null) {
            return 10
        }
        return getKBRoomActivity()!!.kanbanMaxCount
    }
